program oneParticle
  ...
  !dt=0.0000001  
  real, parameter :: xMax=15, xMin=-15, dx=0.1,dt=0.0001, tMax=8.0, sigma=0.5, xNot=0, omegaSquare=16,m=1
  ...
  do timeStep=1,maxT-1
     ...
     if(timeStep == 1 ) then
        Varray=0
     elseif (timeStep == maxT/16) then
        call doubleSlitPotential(Varray)
     elseif (timeStep == maxT/16 + maxT/128) then
        Varray=0
     end if
